...
$('.eqLogicAction[data-action=addEquipement]').on('click', function () {
  bootbox.confirm(
    "<form id='infos' class='form-horizontal'><fieldset>\
...
$('body')
  .undelegate('.icone .iconeOn[data-l1key=chooseIcon]', 'click')
  .delegate('.icone .iconeOn[data-l1key=chooseIcon]', 'click', function () {
...
$('body')
  .undelegate('.icone .iconeAttrOn[data-l2key=iconOn]', 'click')
  .delegate('.icone .iconeAttrOn[data-l2key=iconOn]', 'click', function () {
    $(this).empty();
  });
  ...
  jeedom.eqLogic.getCmd({
    ...
    success: function (data) {
      ...
      for (i = 0; i < data.length; i++) {
        data[i].configuration.options = $('.actionOptions[data-cmd_id=' + data[i].id + ']').getValues(
          '.expressionAttr'
        );
        data[i].type = 'action';
...
function addCmdToTable(_cmd, _type) {
  ...
  if (_cmd.configuration.type == 'multimedia') {
    ...
    tr += '<td class="action">';
    tr +=
      '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-type="' +
      _cmd.type +
      '" data-l2key="action"  style="margin-bottom : 5px;width : 80%; display : inline-block;">';
    tr +=
      '<a class="btn btn-success btn-sm listAction" data-type="action_alarm" title="{{Sélectionner un mot-clé}}"><i class="fas fa-tasks"></i></a>';
    tr +=
      '<a class="btn btn-default btn-sm cursor listCmdActionOn" data-type="' +
      _cmd.type +
      '" data-input="action" style="margin-left : 5px;"><i class="fas fa-list-alt "></i></a>';
    ...
    tr +=
      '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="type" value="custom" style="display:none" >';
    tr += '</div>';
    tr += '</div></span></td>';
    tr += '<td style="width : 180px;">';
    tr +=
      '<span><input type="color" class="cmdAttr" data-l1key="configuration" data-l2key="color" value="#ffffff" />{{ Couleur}}<br/></span> <br/>';
    tr += '</td>';
    tr += '<td class="action">';
    tr +=
      '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-type="action" data-l2key="action"  style="margin-bottom : 5px;width : 80%; display : inline-block;">';
    tr +=
      '<a class="btn btn-success btn-sm listAction" data-type="action_alarm" title="{{Sélectionner un mot-clé}}"><i class="fas fa-tasks"></i></a>';
    tr +=
      '<a class="btn btn-default btn-sm cursor listCmdActionOn" data-type="action" data-input="action" style="margin-left : 5px;"><i class="fas fa-list-alt "></i></a>';
    tr += '<div class="col-lg-6 actionOptions" data-cmd_id="' + init(_cmd.id) + '">';
    tr += jeedom.cmd.displayActionOption(init(_cmd.configuration.action, ''), _cmd.configuration.options[0].options);
    tr += '</div>';
    tr += '</td>';
